...
// Mocking data - In real app, filter for kids items
export const products = merchProducts.map((p, i) => ({ ...p, id: i + 1, priceValue: p.priceValue ?? Number(String(p.price).replace(/[^0-9]/g, '')), productType: p.productType ?? 'kids', inStock: p.inStock ?? true }));
...
const KidsCollection = () => {
    ...
    const allProducts = [...products, ...products.map(p => ({
        ...
        name: p.name + " (Special)",
        priceValue: p.priceValue + 150,
        price: `₹${p.priceValue + 150}`,
        inStock: !p.inStock,
        badge: p.badge ? null : "Offer"
    }))];
        ...
        <>
            ...
            <main className="pt-24 pb-16 px-4 md:px-8 max-w-[1440px] mx-auto">
                ...
                <div className="text-center mb-12">
                    <h1 className="text-3xl md:text-4xl font-light tracking-wide mb-4 uppercase">Kids Collection</h1>

                    {/* Optional Description or Icon */}
                    <div className="flex justify-center mb-4">
                        <img src="https://placehold.co/100x100?text=Kids" alt="" className="w-12 h-12 rounded-full opacity-0" />
                    </div>
                </div>
